...
class OutputMgr {
  ...
  getPauseStr(response) {
    if (this.pauseTime)
      return ` <break time="${this.pauseTime}"/> `;
    else
      return null;
  }
  queuePause(response) {
...
class InputMgr {
  ...
  _processIntent(requ, resp, goalName, intentResolveCB) {
    ...
    return this._followGoals(response, goalName, intentResolveCB)
      .then(()=>{
        outputMgr.sendFromQueue(resp, response);
        outputMgr.initialize();       // clear buffers
        outputMgr.clearSpeaking();
      });
  };

  processAllIntents(requ, resp, idName, intentParams, intentDefArr) {
    ...
    this.convoEngine.platforms.getSlotsFromReq(requ).forEach((slotName)=>{
    ...
    var tmpResp = new Response(this.convoEngine, requ, resp);
    var ndx=0;
    while (true) {
...
class PlatformMgr {
  ...
  regIntent(name, params, cb) {
    this.platforms.forEach(p => {
      p.regIntent(name, params, (requ,resp)=>{
        requ.platform = p;
        return cb(requ,resp);
      });
    });
  ...
  getSlotsFromReq(requ) {
    if (!requ.platform) {
      console.log('ERROR: Expecting Request to have platform attached to it!');
      return null;
    }
    return requ.platform.getSlotsFromReq(requ);
  }
}
...
class ConversationEngine {
  ...
  constructor(appName, platforms) {
    ...
    this.platforms.onError(( exceptionMsg, requ, resp ) => {
      console.log(requ);
      console.log(resp);
      this.outputMgr.sendFromQueue(resp, null, 'Sorry an error occurred ' + exceptionMsg);
    });
    this.platforms.onLaunch( function( requ, resp ) {
      return convoEngine.inputMgr._processIntent(requ, resp, null, (response)=>{
  ...
  _registerIntentDef(userSpeech, intentDefArr) {
    ...
    this.platforms.regIntent(idName, intentParams, (requ, resp) => {
      return this.inputMgr.processAllIntents(requ, resp, idName, intentParams, intentDefArr);
    });
  }
  ...
  setPauseTime(_pauseTime) {
    this.outputMgr.pauseTime = _pauseTime;
  }
