...
class ScriptParser {
  ...
  static forSlots(inputTypes, userSpeech) {
      ...
      var literalSampleValuesStore = {
        get: (inStr)=>{
          ...
          if (inputTypes[inStr] && inputTypes[inStr].sampleValues) {
            ...
            inputTypes[inStr] = 'AMAZON.LITERAL';
          }
...
class InputMgr {
  ...
  _processIntent(response, goalName, intentResolveCB) {
    ...
    return this._followGoals(response, goalName, intentResolveCB)
      .then(()=>{
        outputMgr.sendFromQueue(response.platReq);
  ...
  processAllIntents(platReq, idName, intentParams, intentDefArr) {
    ...
    if (intentDefArr.length==1) {
      ...
      return this._processIntent(response, intentDefArr[0].goal, intentDefArr[0].resolve);
    }
    ...
    while (true) {
      ...
      for (var ndx2 = 0; ndx2 < intentDefArr.length; ndx2++) {
        ...
        if (intentDef.goal && intentDef.goal == currentGoalState.key) {
          return this._processIntent(response, intentDef.goal, intentDef.resolve);
        }
      }
    }
...
class ConversationEngine {
  ...
  constructor(appName, platforms) {
    ...
    this.platforms.onLaunch( function( platReq ) {
      var response = convoEngine.inputMgr._getResponse(platReq);
      return convoEngine.inputMgr._processIntent(response, null, (response)=>{
  ...
  _registerIntentDef(userSpeech, intentDefArr) {
    ...
    userSpeech = ScriptParser.forSlots(this.convo.inputTypes, userSpeech);
