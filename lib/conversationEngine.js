...
class InputMgr {
  ...
  _followGoals(response, tgtGoal, tgtResolve) {
    ...
    var listGoals = (action)=>{console.log(action + ' goals to meet: ', convo.getGoals(response));}
    var topGoal = (goalNdx)=>{listGoals('top'); return convo.mostRecentGoal(response, goalNdx);}
...
class ConversationEngine {
  ...
  getGoals(response) {
    var goals = response.get(convoGoals);
    if (!goals) goals = '';
    if (goals == '') return [];
    return goals.split(';')
  }
  setGoals(response, goalArr) {
    response.set(convoGoals, goalArr.join(';'));
  }
  addGoal(response, goal) {
    console.log('--> Goal added: ' + goal);
    var goalsToMeet = this.getGoals(response, convoGoals);
    goalsToMeet.push(goal);
    this.setGoals(response, goalsToMeet);
