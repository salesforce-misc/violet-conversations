...
var _getUserSpeechItemSig = function(userSpeechItem, inputTypes) {
  ...
  sigItems = sigItems.map((item, ndx)=>{
    ...
    // console.log(`::::: added mapping: ${item}->${typeMappings[item]}`)
...
class IntentMgr {
  ...
  _processIntent(response, intentDef) {
    ...
    response.platReq.getInputTypes().forEach((reqName)=>{
      ...
      console.log(`store upgrade - requstSlots->sessionStore: ${reqName}->${storeName}: `, upgradeVal);
    });
...
class ConversationEngine {
  ...
  loadFlowScript(scriptPath, models) {
    try {
      ...
      console.log('Error while trying to load flow script', e);
    }
  }
  ...
  loadExpectings(expectingsPaths) {
    try {
      ...
      this.expectings = doc;
      // console.log(`convoEngine.expectings: `, this.expectings);
    } catch (e) {
      console.log('Error while trying to load expectings map', e);
    }
  }
