...
class PlatformMgr {
  ...
  setSpokenRate(_rate) {
    this.platforms.forEach(p => {
      p.getOutputMgr().spokenRate = _rate;
    });
  }
...
class ConversationEngine {
  ...
  constructor(appName, platforms) {
    ...
    this.platforms.onError(( exceptionMsg, platReq ) => {
      console.log(requ);
      console.log(resp);
      platReq.platform.outputMgr.sendFromQueue(platReq, null, 'Sorry an error occurred ' + exceptionMsg);
    });
    this.platforms.onLaunch( function( platReq ) {
      var response = convoEngine.intentMgr._getResponse(platReq);
      return convoEngine.intentMgr._processIntent(response, null, (response)=>{
  ...
  setSpokenRate(_rate) {
    this.platforms.setSpokenRate(_rate);
  }
