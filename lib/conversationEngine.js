...
class InputMgr {
  ...
  _followGoals(response, tgtGoal, tgtResolve) {
    ...
    var topGoal = (goalNdx)=>{listGoals('top'); return top(convo.getGoals(response), goalNdx);}
    var removeGoal = (goal)=>{listGoals('remove'); var currGoals=convo.getGoals(response); remove(currGoals, goal); convo.setGoals(response, currGoals);}
