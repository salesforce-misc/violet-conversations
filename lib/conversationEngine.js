...
class ConversationEngine {
  ...
  constructor(appName, cfg) {
    ...
    this.expectings = {}; // deprecated use contentMap instead
  ...
  // @deprecated use addToContentMap instead
   ...
   * @deprecated use loadForContentMap instead
   */
  ...
  addToContentMap(choiceId, expectArr) {
    if (this.contentMap[choiceId]) {
      this.contentMap[choiceId] = this.contentMap[choiceId].concat(expectArr);
    } else {
      this.contentMap[choiceId] = expectArr;
    }
  }

  _importIntoObj(dstObj, srcObj) {
    Object.keys(srcObj).forEach(key => {
      if (dstObj[key]) {
        var srcIsArr = Array.isArray(srcObj[key]);
        var dstIsArr = Array.isArray(dstObj[key]);
        if (!srcIsArr && !dstIsArr) {
          this._importIntoObj(dstObj[key], srcObj[key]);
        } else if (srcIsArr && dstIsArr) {
          // CAUTION: Array.concat does not change the dst, Array.push does and is
          //          more performant
          // CAUTION: Array.push does not work if the source array is long (~100k+
          //          elements - it will give a stackoverflow error) <-- we don't
          //          expect this here
          dstObj[key].push(...srcObj[key]);
        } else {
          //  if (!srcIsArr && dstIsArr) OR
          //  if (srcIsArr && !dstIsArr)
          warn(`Cant import object when source and destination are of different types`);
        }
      } else
  ...
  /**
   * Reads a file containing a list of contentMap from the given path and adds
   * those options to the model.
   *
   * Expectings are provided as key-value pairs with the keys representing ids
   * in the CFL script and the values representing options that the script could
   * expect from the user.
   *
   * @param {string} contentMapPath - path to the contentMap file (consisting
   * of a yaml based data)
   */
  loadForContentMap(contentMapPath, namespace = null) {
    try {
      var loadedDoc = yaml.safeLoad(fs.readFileSync(contentMapPath, 'utf8'));
      this._importIntoObj(this.contentMap, loadedDoc);
      // debug(`convoEngine.contentMap: `, this.contentMap);
    } catch (e) {
      console.error('Error while trying to load expectings map', e);
    }
  }

  static _getFromContentMap(keys, cmEntry) {
    console.log('...', keys, cmEntry);
    // if (keys.length == 1 && keys[0]=='add') console.log('o', cmEntry); // <!-- why is cmEntry empty???
    // iterate and see if any of the keys match
    for (var key of keys) {
      if (!cmEntry[key]) continue;
      // cmEntry[key] is either an array (what we want) or an object that is another filter
      if (Array.isArray(cmEntry[key]))
        return cmEntry[key];
      else
        return ConversationEngine._getFromContentMap(keys, cmEntry[key]);
    }
    return null; // no match
  }
  // similar to _getFromContentMap, but this works on arrays - especially useful
  // for expecting statements
  static _extendArrFromContentMap(givenArr, keys, cmEntry, extendTx) {
    // iterate and see if any of the keys match
    for (var key of keys) {
      if (!cmEntry[key]) continue;
      // cmEntry[key] is either an array (what we want) or an object that is another filter
      if (Array.isArray(cmEntry[key]))
        givenArr = extendTx(givenArr, cmEntry[key])
      else
        givenArr = ConversationEngine._extendArrFromContentMap(givenArr, keys, cmEntry[key], extendTx);
    }
    return givenArr;
  }

  getLocaleContent(locale, mappingId) {
    return ConversationEngine._getFromContentMap([locale, mappingId], this.contentMap);
  }
  // Caution: Inputs/utterances are not locale aware
  extendFromContentMap(givenArr, mappingId) {
    var overwriteTx = (inArr, val)=>{
      return val;
    };
    var unionTx = (inArr, val)=>{
      return inArr.concat(val);
    }
    var tx = this.expectingsOverwrite ? overwriteTx : unionTx;
    return ConversationEngine._extendArrFromContentMap(givenArr, [mappingId], this.contentMap, tx)
  }
