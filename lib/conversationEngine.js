...
class InputMgr {
  ...
  processAllIntents(requ, resp, idName, intentParams, intentDefArr) {
    ...
    this.convoEngine.platform.getSlotsFromReq(requ).forEach((slotName)=>{
...
class ConversationEngine {
  ...
  constructor(platform) {
    ...
    this.platform.onError(( exceptionMsg, requ, resp ) => {
      console.log(requ);
      console.log(resp);
      this.outputMgr.sendFromQueue(resp, null, 'Sorry an error occurred ' + exceptionMsg);
    });
    this.platform.onLaunch( function( requ, resp ) {
  ...
  _registerIntentDef(userSpeech, intentDefArr) {
    ...
    this.platform.regIntent(idName, intentParams, (requ, resp) => {
  ...
  _registerIntents({allIntents, userSpeechDef, registeredGoals, keyTypes}) {
    ...
    for (var kt in keyTypes) {
      if (typeof keyTypes[kt] == 'object' && keyTypes[kt].values)
        this.platform.regCustomSlot(keyTypes[kt].type, keyTypes[kt].values);
    }
  ...
  registerIntents() {
    FlowScriptCompiler.compile(this.flowScriptDoc, this.scriptModels, this);
