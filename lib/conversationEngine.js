...
/**
 * Helps the ConversationEngine build an output as provided by the Response class
 *
 * @class
 */
class OutputMgr {
...
/**
 * Helps the ConversationEngine parse the Violet Script. This class would be
 * a set of static methods but adds phraseEquivalents since it is not needed
 * elsewhere.
 *
 * @class
 */
class ScriptParser {
  ...
  forPhraseEquivalents(userSpeech) {
    // return userSpeech;
...
module.exports = class ConversationEngine {
  ...
  _followGoals(response, tgtGoal, tgtResolve) {
    ...
    return p.then(()=>{return utils.promiseWhile(
      ()=>{return this.outputMgr.asked<1 && moreGoalsToSeek == true;},
      ()=>{
        ...
        if (tgtGoalObj.resolve) {
          ...
          this.outputMgr.prompt(response, tgtGoalObj.prompt);
        } else if (tgtGoalObj.ask) {
          this.outputMgr.ask(response, tgtGoalObj.ask);
        } else {
  ...
  _processIntent(requ, resp, goal, resolveCB) {
    ...
    this.outputMgr.setSpeaking();
    this.outputMgr.initialize();
    return this._followGoals(response, goal, resolveCB)
      .then(()=>{
        this.outputMgr.sendFromQueue(resp);
        this.outputMgr.initialize();       // clear buffers
        this.outputMgr.clearSpeaking();
      });
  };

  _registerIntentDef(userSpeech, intentDefArr) {
    userSpeech = this.scriptParser.forPunctuation(userSpeech);
    userSpeech = this.scriptParser.forSlots(this.keyTypes, userSpeech);
    userSpeech = this.scriptParser.forPhraseEquivalents(userSpeech);
    var intentParams = {};
   ...
   */
  addPhraseEquivalents(_phraseEquivalents) {
    this.scriptParser.addPhraseEquivalents(_phraseEquivalents);
  }
