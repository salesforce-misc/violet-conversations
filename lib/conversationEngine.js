...
module.exports = class ConversationEngine {
  ...
  __getResponse(requ, resp) {
    ...
    return new Response(this, requ, resp, paramsStore, sessionStore);
  };

  // this is what gets called every time a user says something
  _processIntent(requ, resp, goal, resolveCB) {
    var response = this.__getResponse(requ, resp);
    this._setSpeaking();
    this._sayInit();
  ...
  _getResponseForDebugging(requ, resp) {return this.__getResponse(requ, resp);}
