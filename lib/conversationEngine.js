...
class ConversationEngine {
  ...
  addFlowScript(script, models) {
    this.flowScripts.push({doc: FlowScriptCompiler.load(script), models});
  }
  ...
  async addFlowScriptEx(script, models, namespace = 'node') {
    this.flowScripts.push({doc: FlowScriptCompiler.load(script), models, namespace});

    var convoEngine = this;
    Object.keys(models).forEach(async (m)=>{
      if (models[m]['initialize']) await models[m]['initialize'](convoEngine);
    });
  }

  async loadFlowScriptEx(scriptPath, models, namespace = 'node') {
    try {
      var script = fs.readFileSync(scriptPath, 'utf8');
      await this.addFlowScriptEx(script, models, namespace);
    } catch (e) {
      console.error('Error while trying to load flow script', e);
    }
  }
  ...
  registerIntents() {
    ...
    this.flowScripts.forEach(s=>{
      console.log(`FlowScriptCompiler.compile(s.models = '${Object.keys(s.models)}', namespace = '${s.namespace}')`)
      FlowScriptCompiler.compile(s.doc, s.models, convoEngine, s.namespace);
    });
