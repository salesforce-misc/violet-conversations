...
class InputMgr {
  ...
  _followGoals(response, tgtGoal, tgtResolve) {
    ...
    var listGoals = (action)=>{console.log(action + ' goals to meet: ', convo.getGoalNames(response));}
    var topGoalName = (goalNdx)=>{listGoals('top'); return convo.mostRecentGoalName(response, goalNdx);}
    var removeGoal = (goal)=>{listGoals('remove'); convo.clearGoal(response, goal);}
    ...
    // console.log('--> Triggered Goal [_followGoals]: ' + tgtGoal);
    var p = null;
    if (tgtResolve)
    ...
    return p.then(()=>{return utils.promiseWhile(
      ...
      ()=>{
        var tgtGoal = topGoalName(goalNdx++);
        ...
        var tgtGoalObj = convo.registeredGoals[tgtGoal];
        lastGoal = tgtGoal;
        if (!tgtGoalObj) {
        ...
        if (tgtGoalObj.resolve) {
          ...
          convo.outputMgr.prompt(response, tgtGoalObj.prompt);
        } else if (tgtGoalObj.ask) {
          convo.outputMgr.ask(response, tgtGoalObj.ask);
        } else {
...
class ConversationEngine {
  ...
  getGoalNames(response) {
    return ShortJSON.arrToArrObjKey(ShortJSON.sjnToArr(response.get(convoGoals)));
  }
  getGoalObjs(response) {
    return ShortJSON.arrToArrObj(ShortJSON.sjnToArr(response.get(convoGoals)));
  }
  setGoals(response, goalObjsArr) {
    response.set(convoGoals, ShortJSON.arrToSJN(ShortJSON.arrObjToArr(goalObjsArr)));
  }
  appendGoal(response, goalName) {
    var sjnArr = response.get(convoGoals);
    response.set(convoGoals, ShortJSON.arrPush(sjnArr, goalName));
  }
  clearGoal(response, goalName) {
    console.log('--> Clearing Goal: ' + goalName);
    var curGoals = response.get(convoGoals);
  ...
  hasGoal(response, goalName) {
    ...
    return ShortJSON.inSJN(response.get(convoGoals), goalName);
  };
  ...
  clearAllGoals(response) {
    this.setGoals(response, []);
  };
  // a LIFO queue
  mostRecentGoalName(response, goalNdx) {
    var top = (arr, ndx)=>{if (arr.length==0 || ndx>=arr.length) return undefined; else return arr[arr.length-ndx-1];}
    return top(this.getGoalNames(response), goalNdx);
  }
  // experimental
