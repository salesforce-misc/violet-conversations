...
/**
 * Helps the ConversationEngine process the output (from Alexa) and call the
...
class InputMgr {
  ...
  _followGoals(response, tgtGoal, tgtResolve) {
    ...
    var listGoals = (action)=>{console.log(action + ' goals to meet: ', convo.goalsToMeet);}
    var topGoal = (goalNdx)=>{listGoals('top'); return top(convo.goalsToMeet, goalNdx);}
    var removeGoal = (goal)=>{listGoals('remove'); return remove(convo.goalsToMeet, goal);}
