...
var debug = require('debug')('conversationEngine'); // to enable run as: DEBUG=conversationEngine OR DEBUG=*
var warn = require('debug')('conversationEngine:warn'); // to enable run as: DEBUG=conversationEngine:warn OR DEBUG=*
...
var _genIntentName = function(roots) {
  ...
  _generatedIntents++;
  if (roots && easeDev) iName += '_' + roots.join('_');
  return iName;
}
  ...
  processAllIntents(platReq, idName, intentParams, intentDefArr) {
    ...
    var findIntentDef = (goalStateToFind)=>{
      for (var intentDefNdx = 0; intentDefNdx < intentDefArr.length; intentDefNdx++) {
        ...
        if (goalStateToFind && intentDef.goal && intentDef.goal == goalStateToFind) return intentDef;

        // the null search, i.e. find me any intentDef with *no* goals attached to it
        if (!goalStateToFind && !intentDef.goal) return intentDef;
      }
      return null;
    };
...
class ConversationEngine {
  ...
  constructor(appName, cfg) {
    ...
    this.platforms.onError(( exceptionMsg, platReq ) => {
      debug(platReq.request);
      debug(platReq.response);
  ...
  addFlowScript(script, models) {
    if (this.flowScriptDoc) debug('Currently only single flow script is supported');
  ...
  loadExpectings(expectingsPaths) {
    try {
      var doc = yaml.safeLoad(fs.readFileSync(expectingsPaths, 'utf8'));
      this.expectings = doc;
  ...
  _registerIntentDef(userSpeech, intentDefArr) {
    ...
    if (!this.convo.intentRegistry[idName]) this.convo.intentRegistry[idName] = {};
