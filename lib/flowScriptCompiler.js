...
class FlowScriptCompiler {
  ...
  static resolveElementForOutlet(response, flowScriptDoc, el, elNode) {
    ...
    case 'sayOne':
      ...
      break;
    case 'decision':
      response.addGoal(el.attribs.id);
      break;
    case 'jump':
      FlowScriptCompiler.resolveElementForOutlet(response, flowScriptDoc, null, flowScriptDoc(el.attribs.target));
      break;
    // app logic integration elements
    case 'resolve':
      var value = el.attribs.value;
