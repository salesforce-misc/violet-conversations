  ...
  static compile(flowScriptDoc, scriptModels, convoEngine) {
    ...
    const decisionNodeHooksForRegistering = (parentGoalId, node, intentTags = globalIntentTags)=>{
      ...
      intentTagNodes.forEach(child=>{
        var rTo = FlowScriptCompiler.decisionChildTypeToRespondTo(flowScriptDoc, child.name(), child.id(), parentGoalId);
        // console.log(`decisionNodeChild id: ${child.attribs.id} `, rTo)
        respondArr.push(rTo);
      });
      return respondArr;
    };
    flowScriptDoc.selector('app').forEach(node=>{
      console.log(`app-id: ${node.id()}`);
      var respondArr = decisionNodeHooksForRegistering(null, node);
      respondArr.forEach(r=>{
        convoEngine.respondTo(r);
      });
    });
    flowScriptDoc.selector('decision').forEach(decisionNode=>{
      console.log(`decision-id: ${decisionNode.id()}`);
      var goalObj = {
      ...
      goalObj.respondTo = decisionNodeHooksForRegistering(decisionNode.id(), decisionNode);

      convoEngine.defineGoal(goalObj);
    });
    flowScriptDoc.selector('dialog').forEach(dialogNode=>{
      console.log(`dialog el: ${dialogNode.id()}`);
      ...
      var respondArr = decisionNodeHooksForRegistering(dialogNode.id(), dialogNode, ['item']);
