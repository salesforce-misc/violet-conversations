...
const debug = require('debug')('flowScriptCompiler'); // to enable run as: DEBUG=flowScriptCompiler
...
class FlowScriptCompiler {
  ...
  static _choiceHooksToRespondTo(flowScriptDoc, convoEngine, decisionChildType, decisionChildId, parentGoalId) {
    decisionChildId = decisionChildId.replace(/\./g, '\\.');
    ...
    if (!intentDef.expecting || intentDef.expecting.length == 0) console.error(`<expecting> not found in ${decisionChildType}#${decisionChildId}`)
