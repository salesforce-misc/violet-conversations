...
class GooglePlatform extends PlatformPlugin {
  ...
  setServerApp(violetRouter) {
    ...
    violetRouter.post('/' + this.endpoint, function (request, response) {
      try {
        ...
        request.slot = (pName) => {
          return platform.app.parameters[pName];
        };
        var shouldEndSession = true;
        response.shouldEndSession = (flag) => { shouldEndSession = flag; }
        response.say = (str) => {
          if (shouldEndSession) platform._tell(str);
          else platform._ask(str);
        }
        ...
        Object.keys(platform.app.parameters).forEach(pName=>{
          sessionStore[pName] = platform.app.parameters[pName];
        });
        let result = platform.intentHandlers[intentName](request, response);
  ...
  getSlotsFromReq(requ) {
    return Object.keys(this.app.parameters);
  }
