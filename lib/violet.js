...
var _promiseWhile = Promise.method(function(condition, action) {
  if (!condition()) return;
...
module.exports = function(app) {
  ...
  const paramsRE   = /\[\[([a-zA-Z_]*)\]\]/;  // from user input
  const paramsGRE  = /\[\[([a-zA-Z_]*)\]\]/g;
  const sessionRE  = /{{([a-zA-Z_]*)}}/;      // from the session
  const sessionGRE = /{{([a-zA-Z_]*)}}/g;
  const storeRE    = /<<([a-zA-Z_\.]*)>>/;    // from permanent store (database, etc)
  const storeGRE   = /<<([a-zA-Z_\.]*)>>/g;
  ...
  var _followGoals = (response, tgtGoal, tgtResolve) => {
    ...
    var resolveGoal = function(tgtResolve, tgtGoal, response) {
      ...
      if (result.next) {  // generator return
        // run the rest of generator [co fortunately allow the genObj as a paramer and not just genFunc]
        return co(result).then((result)=>{
  ...
  var __getResponse = (req, resp) => {
    ...
    var response = {
      ...
      get: (varStr) => {
        ...
        if (varMatch != null)
          return persistentStore.get(varMatch[1]);
      ...
      // persistence support
      load: (objNameStr, keyNameStr, keyVal, filter, orderByString) => {
        ...
        if (objNameMatch != null && keyNameMatch != null) {
          ...
          return persistentStore.load(objNameMatch[1], keyNameMatch[1], keyVal, filter, orderByString)
        } else if (filter != null) {
          console.log('Loading object: ' + objNameMatch[1]);
          return persistentStore.load(objNameMatch[1], null, null, filter, orderByString)
        }
      ...
      goalFilled: (paramName) => {
        return goalFilledByStore('{{' + paramName + '}}', '[[' + paramName + ']]');
      }
    };
    return response;
  };
        ...
        for (var ndx = goalsToMeet.length; ndx--; ) {
          ...
          for (var ndx2 = 0; ndx2 < phraseOptions.length; ndx2++) {
            ...
            if (po.goal == currentGoal) {
              return _processIntent(requ, resp, po.goal, po.responseImplCB);
            }
          }
        }
  ...
  var violet = {
    ...
    respondTo: function(userSpeech, responseImplCB, goal = null) {
      ...
      console.log('registering: ', intentParams);
      app.intent(_genIntentName(), intentParams, (requ, resp) => {
        return _processIntent(requ, resp, goal, responseImplCB);
      });
    }
  }
