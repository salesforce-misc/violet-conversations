...
module.exports = function(app) {
  ...
  var violet = {
    ...
    respondTo: function(userSpeech, responseImplCB, goal = null) {
      ...
      app.intent(_genIntentName(), intentParams, (requ, resp) => {
        ...
        var response = _getResponse(requ, resp);
        _setSpeaking();
        _sayInit();
  ...
  app.launch( function( requ, resp ) {
    var response = _getResponse(requ, resp);
    if (fAlert.length > 0) {
      _sayFinish(resp, response, 'You have an alert.');
      console.log('Alerts: ', fAlert);
      console.log('Goals: ', goalsToMeet);
      return;
    }
    _sayFinish(resp, response, ['Yes. How can I help?', 'Hey. Need me?', 'Yup. I am here.']);
  });
