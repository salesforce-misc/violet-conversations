...
/**
 * Core Violet Module
 * @module violet-conversations/violet
 */
...
/** Used by server - primarily allows for the loading of scripts */
module.exports.server = function() {
  return {
    ...
    loadMultipleScripts: (contextDir, scriptPath, name, alexaRouter) => {
      appName = name;
      var script = violetScript();
      var intentsDef = [];
      scriptPath.forEach(p=>{
        var ps = require(path.join(contextDir, p));
        intentsDef.push(ps.getIntentsDef());
      })
      script.registerIntents(intentsDef);;
      script.setServerApp(alexaRouter);
      return script;
    }
  };
}
/** Used by scripts - instantiated and returns the Violet class */
var violetScript = module.exports.script = function() {
  if (appName == null) {
  ...
  var __getResponse = (requ, resp) => {
    ...
    var response = {
      ...
      /** should the conversation end after the current script response?
  ...
  var _registerIntentDef = (userSpeech, intentDefArr) => {
    ...
    _regIntent(_app,  idName, intentParams, (requ, resp) => {
      ...
      console.log(`No matching response for: ${idName} intentDefArr.length: ${intentDefArr.length} utterances:`, intentParams["utterances"]);
    });
  };
  ...
  // really only used when registering multiple scripts - would be nice to not have this code duplication
  var _consumeIntentDefItems = (idi) => {
    for (var goal in idi.registeredGoals) {
      if (registeredGoals[goal]) console.log('WARN: duplicate goal: ', goal);
      registeredGoals[goal] = idi.registeredGoals[goal];
    }
    for (var typ in idi.keyTypes) {
      if (keyTypes[typ]) console.log('WARN: duplicate type: ', typ);
      keyTypes[typ] = idi.keyTypes[typ];
    }
    allIntents = allIntents.concat(idi.allIntents);
    Object.keys(idi.userSpeechDef).forEach(userSpeechItem=>{
      if (!userSpeechDef[userSpeechItem]) userSpeechDef[userSpeechItem] = [];
      userSpeechDef[userSpeechItem] = userSpeechDef[userSpeechItem].concat(idi.userSpeechDef[userSpeechItem]);
    });
  };
  ...
  var violet = {
    ...
    addKeyTypes: function(_keyTypes) {
      return violet.addInputTypes(_keyTypes);
    },
    ...
    registerIntents: function(intentDef) {
      if (Array.isArray(intentDef)) {
        intentDef.forEach(intentDefItem=>{
          _consumeIntentDefItems(intentDefItem);
        });
        intentDef = null; // we need to reload intentDef so clear it
      }
      if (!intentDef) intentDef = _getIntentsDef();
      _registerIntents(intentDef);
    },

    /*
      params - passed in as an object:
        expecting <-- array of strings that a user could say
        resolve   <-- callback when one of the `expecting` items is said
        goal      <-- (optional) goal during which the above is valid

      alternatively, can pass in params as: userSpeech, resolve, goal
    */
