...
/**
 * Core Violet Module - provides a conversation engine that voice-scripts can
 * take advantage of. The API is designed to allow developers to build
 * sophisticated voice apps. This is supported by allowing (a) for easily
 * building basic logic supported conversation-flows i.e. classically referred
 * to as the view layer in web, mobile, and desktop apps; (b) primitives for
 * grouping user/app interactions (b-i) using goals to enable for what is
 * classically referred to as dialog in many conversation systems,
 * (b-ii) using widgets for generating goals depending on the use-case, such
 * as for supporting lists, and (b-iii) having scripts to allow for further
 * modularization of voice functionalty; (c) plugins for moving out significant
 * parts of complexity that support the voice scripts.
 *
