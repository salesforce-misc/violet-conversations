  ...
  var __getResponse = (requ, resp) => {
    ...
    var response = {
      ...
      clear: (varStr) => {
        ...
        varMatch = paramsRE.exec(varStr);
        if (varMatch != null) {
          console.log('cannot clear an input variable');
          return;
        }
        ...
        if (varMatch != null) {
          console.log('Setting store variable: ' + varMatch[1] + ' <-- ' + val);
          return persistentStore.clear(varMatch[1]);
        }
      ...
      // persistence support
      load: function(objName, keyName, keyVal, filter, queryXtra) {
        if (arguments.length>1) {
          var params = {};
          if (objName)   params.objName   = arguments[0];
          if (keyName)   params.keyName   = arguments[1];
          if (keyVal)    params.keyVal    = arguments[2];
          if (filter)    params.filter    = arguments[3];
          if (queryXtra) params.queryXtra = arguments[4];
          return response.load(params);
        }
        var params = objName;

        // this is deprecated - it supports store parameters to be given as '<<xyz>>'
        var _stripParamCover = (params, key) => {
          var keyMatch = storeRE.exec(params[key]);
          if (keyMatch != null) params[key] = keyMatch[1];
        }
        _stripParamCover(params, 'objName');
        _stripParamCover(params, 'keyName');
      ...
      store: (objName, dataToStore) => {
        var objNameMatch = storeRE.exec(objName);
        if (objNameMatch != null) objName = objNameMatch[1];
