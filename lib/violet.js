...
var violetSvc = function(app) {
  ...
  var _say = function(response, potResponses, params=()=>{return 'err';}) {
    ...
    var getParamsFromStore = function(str, paramsRE, store) {
      var paramsMatch;
      while ((paramsMatch = re.exec(str)) != null) {
        ...
        // input - input string
        str = _interpolate(str, paramsMatch[0], paramsMatch.index, store.get(paramsMatch[1]));
      }
      return str;
    };
    str = getParamsFromStore(str, /{{([a-z]*)}}/g, params);
    str = getParamsFromStore(str, /<<([a-z]*)>>/g, session);
