  ...
  var __getResponse = (req, resp) => {
    ...
    var response = {
      ...
      contains: (varStr) => {
        ...
        if (getResult.matched) return response.__containsUsingMatched(getResult, paramsStore);
      ...
      get: (varStr) => {
        ...
        getResult = response.__matchAgainstStore(varStr, paramsRE);
        if (getResult.matched) return response.__getUsingMatched(getResult, paramsStore);
      ...
      goalFilledByStore: (destParamName, srcParamName) => {
        ...
        if (response.contains(srcParamName)) {
