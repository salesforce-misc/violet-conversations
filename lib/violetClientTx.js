...
module.exports = (violet, srvrInstance) => {
  ...
  violet.outputMgr.sendFromQueue = (resp, spokenRate, response, potResponses) => {
    var outStr = oldSendFromQueue.call(violet.outputMgr, resp, spokenRate, response, potResponses);
    if (!outStr) return;
    violet.broadcast({
      response: outStr
    });
