  ...
  var listWidget = {
    ...
    getItemFromResults: (response, itemNo)=>{
      ...
      if (results == undefined || results == null || !Array.isArray(results))
        return resposne.say(errMgrNotFoundItems);
      if (results.length<itemNo)
        return resposne.say(errMgrNotFoundTgtItem);

      return results[itemNo];
    },

    defineItemInteraction: (goalDefFragment)=>{
      goalDefFragment.goal = `interactWith${widgetType}`;
      violet.defineGoal(goalDefFragment);
    }
