...
module.exports = (prefix)=>{
  ...
  expressApp.set('views', path.join(__dirname, '..', 'web-tooling-views'));
  expressApp.use(express.static(path.join(__dirname, '..', 'web-tooling-views')));
  if (prefix) {
  ...
  var violetSrvr = {
    ...
    displayScriptInitialized: (srvrInstance, script)=>{
      appNames.push(script.getAppName());
      var host = srvrInstance.address().address;
      var port = srvrInstance.address().port;
      if (host=='::') host='localhost'
      console.log(`>>> Script running at: http://${host}:${port}${prefix}/${script.getAppName()}`);
    },
    ...
    loadScript: (scriptPath, appName)=>{
      ...
      expressApp.use(`/${appName}`, appRouter);
