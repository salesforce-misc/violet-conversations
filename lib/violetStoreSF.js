...
var sfStore = {
  ...
  /*
    params: objName, keyName, keyVal, filter, queryXtra
  */
  load:  (params)=>{
    // console.log('sfStore.load: ' + params.objName, sfStore._objProps(params.objName));
    var q = 'SELECT ';
    q+= sfStore._objProps(params.objName).map((p)=>{return _getCompoundNameToStr(p);}).join(', ');
    ...
    return org.query({ query: q }).then(function(resp){
      ...
      resp.records.forEach((rec)=>{
        var recStore = {};
        sfStore._objProps(params.objName).forEach((p)=> {
