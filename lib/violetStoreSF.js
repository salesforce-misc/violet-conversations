...
class VioletStoreSF {
  ...
  load(params) {
    ...
    if (params.query)
      ...
      q+= _dedupe(this._objProps(params).map((p)=>{return _getCompoundNameToStr(p);})).join(', ');
    if (params.objName)
  ...
  search(objName, searchStr) {
    ...
    q+= _dedupe(this._objProps({objName}).map((p)=>{return _getCompoundNameToStr(p);})).join(', ');
